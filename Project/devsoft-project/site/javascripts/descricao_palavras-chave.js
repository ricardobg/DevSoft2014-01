  ...
  $.each(data, function(key, field) {
    ...
    if (!vazio)
    {
      ...
      if ((igual != -1 && key < igual) || igual == -1)
        if (valores["Descrição:"] != null && valores["Descrição:"] != "")
        {
        	// le palavras
        	var palavras_sep = valores["Descrição:"].toString().toLowerCase().replace(/[\.,:•\*]/g," ").replace(/\-(?=[^a-z]{0,})/g," ").replace(/\s{2,}/g," ").split(" ");
          ...
          var termos_sep = valores["Descrição:"].toString().toLowerCase().match(/([a-z\u00C0-\u00ff]+)|([\.\,])/g);
          if (termos_sep == null)
            return 1;
