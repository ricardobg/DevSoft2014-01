...
14:55 - [política] Lei de incentivo dao esporte é aprovada no senado
...
# Assumindo que hora signifique hora:minuto

res = str.scan(/^[ ]*(\d\d:\d\d) - \[(.*)\] (.*)$/)
res.each do |r|
	puts r[1] + ": " + r[2] + " (" + r[0] + ")"
end
...
# Seu código aqui...
n_lines = str.scan(/^.*$/).length
puts "Existem #{n_lines} linhas."

# 3) Quantas notícias de economia de falam sobre o dólar?

# Seu código aqui...
n_dolar = str.scan(/^.*dólar.*$/).length
puts "#{n_dolar} notícias falam sobre o dólar."

# 4) Imprimir todos os horários das notícias que contém a palavra golfe.
golfe_noticias = str.scan(/^[ ]*(\d\d:\d\d).*golfe.*$/)
golfe_noticias.each do |golfe|
	puts golfe[0]
end
...
maiusculas = str.gsub(/^[ ]*\d\d:\d\d - \[.*\]/) do |tema|
  tema.upcase
end
print maiusculas
